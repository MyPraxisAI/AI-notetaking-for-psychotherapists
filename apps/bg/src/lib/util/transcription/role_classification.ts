...
export async function classifySpeakerRoles(
  ...
  if (!transcription.contentJson?.segments || transcription.contentJson.segments.length === 0) {
    console.warn('No segments to classify in the transcription');
    return transcription;
  }

  // Format the transcript for classification
  const transcript = transcription.contentJson.segments
    .map(segment => {
  ...
  const updatedSegments = transcription.contentJson.segments.map(segment => {
  ...
  transcription.contentJson.segments = updatedSegments;
  transcription.contentJson.classified = true;  
